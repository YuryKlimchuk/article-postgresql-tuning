...
В этой части — подготовка плацдарма:
...
Весь код для запуска лежит в папке `docker/` проекта. Нам понадобится Docker и Docker Compose.

### Что внутри

```
docker/
├── .env                  # Учётные данные и порты
...
Подключаемся к малой БД (порт 5432):
...
Ключевая особенность: у пользователя **два идентификатора** — числовой `id` (BIGSERIAL) и `uuid`. Это сделано специально: в части 3 мы сравним скорость JOIN-ов по разным типам внешних ключей.
...
**Два внешних ключа на users** — это центральный учебный элемент схемы. `user_id` ссылается на числовой первичный ключ, `user_uuid` — на UUID. В части 3 мы будем делать JOIN по каждому из них и измерять разницу.
...
*Исходный код: [github.com/...](https://github.com/...)*

...
**Обратная сторона индексов:** каждый индекс замедляет `INSERT`, `UPDATE` и `DELETE` — PostgreSQL должен обновить не только таблицу, но и все индексы. На нашей схеме 12 индексов — каждая вставка в `transactions` обновляет 5 индексов. В продакшене держите только те индексы, которые реально используются запросами. Неиспользуемый индекс — чистый минус к производительности записи.

**Итог:** B-tree покрывает 99% случаев, но знание альтернатив (Hash для UUID, GIN для JSONB, BRIN для временных рядов) — то, что отличает Middle от Senior. Выбирайте тип индекса под задачу, а не наоборот.

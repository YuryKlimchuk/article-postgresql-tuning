...
**BRIN в деле:** сравним с B-tree на диапазонном запросе:

```sql
-- B-tree: Index Scan, 107 MB индекс
EXPLAIN (ANALYZE, BUFFERS)
SELECT * FROM transactions WHERE transaction_time > NOW() - INTERVAL '30 days';

-- BRIN: Bitmap Heap Scan, 56 KB индекс
DROP INDEX idx_transactions_time;  -- удаляем B-tree
CREATE INDEX idx_tx_time_brin ON transactions USING brin(transaction_time);
...
BRIN может быть чуть медленнее на точечных запросах, но:
- Занимает в 2000 раз меньше места
- Быстро отбрасывает нерелевантные диапазоны страниц
- Идеален для append-only таблиц (логи, временные ряды)

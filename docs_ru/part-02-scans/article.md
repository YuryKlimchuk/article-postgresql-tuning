...
> **Пояснение про read=2:** индекс `idx_tx_card_amount` только что создан и ещё не в кэше (`shared_buffers`). При повторном запуске вы увидите только `hit`. Это нормально — новый индекс всегда начинается с «холодного» кэша.
...
**На большой БД разница будет радикальной** — таблица и её индексы не влезают в кэш, поэтому Index Scan будет читать и индекс, и таблицу с диска, а Index Only Scan — только индекс.

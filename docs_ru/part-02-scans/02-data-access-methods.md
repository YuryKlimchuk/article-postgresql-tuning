...
## Советы пилота: что вынести из Части 2
...
До сих пор мы рассматривали запросы к одной таблице. Но реальные приложения почти всегда работают с несколькими таблицами через JOIN.

В **Части 3** мы увидим, как PostgreSQL соединяет таблицы:
- **Nested Loop Join** — для маленьких выборок, когда для каждой строки левой таблицы ищем совпадение в правой
- **Hash Join** — строим хеш-таблицу в памяти, критичен параметр `work_mem`
- **Merge Join** — когда оба набора уже отсортированы
- И главный сюрприз: **JOIN по `INT` работает в 10 раз быстрее, чем JOIN по `UUID` или `VARCHAR`** — и мы докажем это на реальных цифрах из нашей схемы
